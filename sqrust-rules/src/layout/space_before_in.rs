...
pub struct SpaceBeforeIn;

impl Rule for SpaceBeforeIn {
    fn name(&self) -> &'static str {
        "Layout/SpaceBeforeIn"
    }
...
fn find_violations(source: &str, rule_name: &'static str) -> Vec<Diagnostic> {
    ...
    while i < len {
        ...
        // Need at least kw_len bytes remaining to match IN.
        if i + kw_len > len {
            i += 1;
            continue;
        }

        // Word-boundary check before IN.
        let before_ok = i == 0 || !is_word_char(bytes[i - 1]);
        if before_ok && bytes[i..i + kw_len].eq_ignore_ascii_case(kw) {
            let after = i + kw_len;
            // Word-boundary check: char right after IN must not be alphanumeric/underscore.
            let after_ok = after >= len || !is_word_char(bytes[after]);
            if after_ok && after < len && bytes[after] == b'(' && !skip[after] {
                // IN immediately followed by '(' with no space — flag it.
                let col = i - line_start + 1;
                diags.push(Diagnostic {
                    rule: rule_name,
                    message:
                        "IN( should be IN ( — add a space between IN and the opening parenthesis"
                            .to_string(),
                    line,
                    col,
                });
