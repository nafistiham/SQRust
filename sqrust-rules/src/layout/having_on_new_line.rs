...
pub struct HavingOnNewLine;

impl Rule for HavingOnNewLine {
    fn name(&self) -> &'static str {
        "Layout/HavingOnNewLine"
    }
...
fn find_violations(source: &str, rule_name: &'static str) -> Vec<Diagnostic> {
    ...
    for (line_idx, line) in source.split('\n').enumerate() {
        ...
        if !line_contains_group_by(&line_upper, line_offset, &skip) {
            continue;
        }

        if !line_contains_keyword(&line_upper, line_offset, b"HAVING", 6, &skip) {
            continue;
        }

        let col = line_upper.find("GROUP").unwrap_or(0) + 1;
        diags.push(Diagnostic {
            rule: rule_name,
            message: "HAVING clause is on the same line as GROUP BY; prefer placing HAVING on a new line for readability".to_string(),
            line: line_num,
            col,
        });
    }
...
fn line_contains_group_by(line_upper: &str, line_offset: usize, skip: &[bool]) -> bool {
    ...
    while i + 5 <= len {
        // Match GROUP followed by optional whitespace and BY
