...
fn find_violations(source: &str, rule_name: &'static str) -> Vec<Diagnostic> {
    ...
    if !contains_keyword(&upper, b"GROUP") {
        return Vec::new();
    }
    ...
    // Find the position right after GROUP BY (past the BY token)
    let after_group_by = find_after_two_word_keyword(&upper, group_by_start);

    // Find end of GROUP BY clause — next major clause or end of statement
    let clause_end = find_clause_end(&upper, after_group_by);

    let region = &source[after_group_by..clause_end];

    let mut diags = Vec::new();

    // Calculate line number offset for diagnostics
    let lines_before = source[..after_group_by].chars().filter(|&c| c == '\n').count();
...
/// Find the position right after the two-word keyword (e.g. GROUP BY).
...
/// Returns the start position of the GROUP BY clause in the source.
...
/// Find the end of the GROUP BY clause block.
fn find_clause_end(upper: &str, from: usize) -> usize {
    let bytes = upper.as_bytes();
    let len = bytes.len();

    const TERMINATORS: &[&[u8]] = &[
        b"LIMIT", b"HAVING", b"ORDER", b"UNION", b"INTERSECT", b"EXCEPT",
    ];
