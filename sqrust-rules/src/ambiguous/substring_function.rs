...
fn message_for(name: &str) -> String {
    match name {
        "substr" => {
            "SUBSTR() is a non-standard alias — use SUBSTRING(str, start, length) for maximum portability"
                .to_string()
        }
        "mid" => {
            "MID() is MySQL-specific — use SUBSTRING(str, start, length) for portable substring extraction"
                .to_string()
        }
        _ => unreachable!(),
    }
...
fn walk_expr(
    ...
    match expr {
        Expr::Function(func) => {
            ...
            // counters[0] = substr occurrences, counters[1] = mid occurrences
            ...
            if let Some(idx) = func_key {
                ...
                let func_name_str = if idx == 0 { "SUBSTR" } else { "MID" };
