...
pub struct YearMonthDayFunction;

/// Function specs: (uppercase name, message).
const FUNCTIONS: &[(&str, &str)] = &[
    (
        "YEAR",
        "YEAR() is not ANSI SQL; use EXTRACT(YEAR FROM col) for standard SQL",
    ),
    (
        "MONTH",
        "MONTH() is not ANSI SQL; use EXTRACT(MONTH FROM col) for standard SQL",
    ),
    (
        "DAY",
        "DAY() is not ANSI SQL; use EXTRACT(DAY FROM col) for standard SQL",
    ),
    (
        "HOUR",
        "HOUR() is not ANSI SQL; use EXTRACT(HOUR FROM col) for standard SQL",
    ),
    (
        "MINUTE",
        "MINUTE() is not ANSI SQL; use EXTRACT(MINUTE FROM col) for standard SQL",
    ),
    (
        "SECOND",
        "SECOND() is not ANSI SQL; use EXTRACT(SECOND FROM col) for standard SQL",
    ),
];

impl Rule for YearMonthDayFunction {
    fn name(&self) -> &'static str {
        "Ambiguous/YearMonthDayFunction"
    }
