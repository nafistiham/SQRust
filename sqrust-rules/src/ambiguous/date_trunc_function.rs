...
const FUNCTIONS: &[(&str, &str)] = &[
    (
        "DATE_TRUNC",
        "DATE_TRUNC() is PostgreSQL/DuckDB-specific; syntax and supported units vary across databases",
    ),
    (
        "DATE_FORMAT",
        "DATE_FORMAT() is MySQL-specific; use TO_CHAR() (Oracle/PostgreSQL) or FORMAT() (SQL Server) for formatting dates",
    ),
    (
        "TRUNC",
        "TRUNC() for date truncation is Oracle/PostgreSQL-specific; use DATE_TRUNC() or DATETRUNC() depending on dialect",
    ),
];
