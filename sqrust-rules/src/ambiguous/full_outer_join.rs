...
fn check_set_expr(
    ...
    match expr {
        SetExpr::Select(sel) => {
            for twj in &sel.from {
                recurse_table_factor(&twj.relation, source, occurrence, diags);
                for join in &twj.joins {
                    if matches!(join.join_operator, JoinOperator::FullOuter(_)) {
                        let (line, col) =
                            find_nth_keyword_position(source, "FULL", *occurrence);
                        *occurrence += 1;
                        diags.push(Diagnostic {
                            rule: "Ambiguous/FullOuterJoin",
                            message: "FULL OUTER JOIN may produce unintentionally large results; verify this is intentional".to_string(),
                            line,
                            col,
                        });
                    }
