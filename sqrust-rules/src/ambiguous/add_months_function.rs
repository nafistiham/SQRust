...
pub struct AddMonthsFunction;

/// Function specs: (uppercase name, message).
const FUNCTIONS: &[(&str, &str)] = &[
    (
        "ADD_MONTHS",
        "ADD_MONTHS() is Oracle-specific; use standard interval arithmetic (date + INTERVAL n MONTH) for portable SQL",
    ),
    (
        "MONTHS_BETWEEN",
        "MONTHS_BETWEEN() is Oracle-specific; use DATEDIFF or interval subtraction depending on your dialect",
    ),
];

impl Rule for AddMonthsFunction {
    fn name(&self) -> &'static str {
        "Ambiguous/AddMonthsFunction"
    }
