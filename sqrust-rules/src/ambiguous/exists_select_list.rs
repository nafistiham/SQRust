...
pub struct ExistsSelectList;

impl Rule for ExistsSelectList {
    fn name(&self) -> &'static str {
        "Ambiguous/ExistsSelectList"
    }
...
fn collect_from_select(select: &Select, ctx: &FileContext, diags: &mut Vec<Diagnostic>) {
    // Check SELECT projection expressions for nested EXISTS.
...
/// Returns true when the SELECT list inside an EXISTS subquery contains a
/// column reference (identifier or compound identifier), which is misleading
/// because the database ignores the SELECT list entirely.
fn has_column_reference_projection(select: &Select) -> bool {
    for item in &select.projection {
        match item {
            // EXISTS (SELECT col ...) or EXISTS (SELECT a.b ...)
            SelectItem::UnnamedExpr(Expr::Identifier(_))
            | SelectItem::UnnamedExpr(Expr::CompoundIdentifier(_)) => return true,
            // EXISTS (SELECT col AS x ...)
...
/// Recursively walks expressions looking for EXISTS subqueries whose SELECT
/// list contains column references.
fn check_expr(expr: &Expr, ctx: &FileContext, diags: &mut Vec<Diagnostic>) {
    match expr {
        Expr::Exists { subquery, .. } => {
            // Inspect the EXISTS subquery body.
            if let SetExpr::Select(sel) = subquery.body.as_ref() {
                if has_column_reference_projection(sel) {
                    let (line, col) = find_keyword_position(&ctx.source, "EXISTS");
                    diags.push(Diagnostic {
                        rule: "Ambiguous/ExistsSelectList",
                        message: "SELECT list inside EXISTS is ignored — use EXISTS (SELECT 1 ...) or EXISTS (SELECT * ...) for clarity".to_string(),
                        line,
                        col,
                    });
                }
            }
            // Recurse into the EXISTS subquery to catch nested cases.
            collect_from_query(subquery, ctx, diags);
        }

        // Recurse through all other expression kinds.
