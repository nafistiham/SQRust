...
pub struct CaseWhenCount {
    /// Maximum number of WHEN clauses allowed per CASE expression.
    /// CASE expressions with more branches than this are flagged.
...
impl Rule for CaseWhenCount {
    ...
    fn check(&self, ctx: &FileContext) -> Vec<Diagnostic> {
        ...
        // Track how many CASE keywords we've consumed so we can map each
        // violation to the correct source position.
        let mut case_occurrence: usize = 0;

        for stmt in &ctx.statements {
            if let Statement::Query(query) = stmt {
                check_query(query, self.max_when_clauses, ctx, &mut case_occurrence, &mut diags);
            }
        }
    ...
    match expr {
        Expr::Case {
            ...
            *occurrence += 1;
            ...
            // Recurse into each condition and result expression.
