...
impl Rule for OnConflictClause {
    fn name(&self) -> &'static str {
        "Lint/OnConflictClause"
    }
    ...
    fn check(&self, ctx: &FileContext) -> Vec<Diagnostic> {
        ...
        for (line, col) in find_two_word_keyword(source, "on", "conflict", &skip) {
            diags.push(Diagnostic {
                rule: self.name(),
                message: "ON CONFLICT is PostgreSQL/SQLite-specific upsert syntax; use MERGE for standard SQL or check dialect-specific INSERT ... ON DUPLICATE KEY UPDATE (MySQL)"
                    .to_string(),
                line,
                col,
            });
