...
impl Rule for CallStatement {
    ...
    fn check(&self, ctx: &FileContext) -> Vec<Diagnostic> {
        ...
        for (_offset, line, col) in find_keyword_with_offset(source, "call", &skip) {
            diags.push(Diagnostic {
                rule: self.name(),
                message: "CALL statement invokes a stored procedure; consider using a SELECT or \
                          DML statement for portable SQL"
                    .to_string(),
                line,
                col,
            });
