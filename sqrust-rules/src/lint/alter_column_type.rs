        ...
        for stmt in &ctx.statements {
            if let Statement::AlterTable { operations, .. } = stmt {
                for op in operations {
                    ...
                    if is_type_change {
                        let (line, col) =
                            find_keyword_position(source, &source_upper, "ALTER");
                        diags.push(Diagnostic {
                            rule: self.name(),
                            message: "Changing a column's data type can silently truncate \
                                      existing data — review carefully before running in production"
                                .to_string(),
                            line,
                            col,
                        });
...
/// Finds the 1-indexed (line, col) of the first occurrence of `keyword`
/// (already uppercased) in `source_upper` with word boundaries on both sides.
