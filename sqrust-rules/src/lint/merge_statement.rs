    ...
    fn check(&self, ctx: &FileContext) -> Vec<Diagnostic> {
        ...
        for stmt in &ctx.statements {
            if let Statement::Merge { .. } = stmt {
                let (line, col) =
                    find_nth_keyword(source, &source_upper, "MERGE", occurrence);
                occurrence += 1;
                diags.push(Diagnostic {
                    rule: self.name(),
                    message: "MERGE statement is not supported by all databases \
                               — verify compatibility or use INSERT ... ON CONFLICT \
                               / UPSERT instead"
                        .to_string(),
                    line,
                    col,
