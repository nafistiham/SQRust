...
impl Rule for DropTableIfExists {
    ...
    fn check(&self, ctx: &FileContext) -> Vec<Diagnostic> {
        ...
        // Track how many DROP TABLE violations we have found so we can locate each
        // occurrence in the source text.
        let mut drop_table_occurrence: usize = 0;
        ...
        for stmt in &ctx.statements {
            ...
            {
                if *object_type == ObjectType::Table && !if_exists {
                    let (line, col) =
                        find_nth_keyword(source, &source_upper, "DROP", drop_table_occurrence);
                    drop_table_occurrence += 1;
                    diags.push(Diagnostic {
                        rule: self.name(),
                        message: "DROP TABLE without IF EXISTS may fail if the table does not exist"
                            .to_string(),
                        line,
                        col,
                    });
