...
impl Rule for DropSchemaStatement {
    ...
    fn check(&self, ctx: &FileContext) -> Vec<Diagnostic> {
        ...
        // Track occurrence indices separately for SCHEMA and DATABASE drops so
        // that each can be located independently in the source text.
        let mut schema_occurrence: usize = 0;
        let mut database_occurrence: usize = 0;

        for stmt in &ctx.statements {
            if let Statement::Drop { object_type, .. } = stmt {
                match object_type {
                    ObjectType::Schema => {
                        let (line, col) = find_nth_keyword(
                            source,
                            &source_upper,
                            "DROP",
                            schema_occurrence,
                        ...
                        diags.push(Diagnostic {
                            rule: self.name(),
                            message:
                                "DROP SCHEMA/DATABASE is irreversible; ensure you have a backup"
                                    .to_string(),
                            line,
                            col,
                        });
