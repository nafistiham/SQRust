        ...
        for stmt in &ctx.statements {
            if let Statement::Update { selection, .. } = stmt {
                if selection.is_none() {
                    // Find the position of the UPDATE keyword in source (case-insensitive,
                    // word-boundary check).
                    let (line, col) = find_keyword_position(source, &source_upper, "UPDATE");
