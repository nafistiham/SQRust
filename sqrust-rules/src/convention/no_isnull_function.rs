...
pub struct NoIsnullFunction;

const MESSAGE: &str =
    "ISNULL() is SQL Server/Sybase-specific; use COALESCE() for two-argument null replacement or the IS NULL predicate for null checks";

impl Rule for NoIsnullFunction {
    fn name(&self) -> &'static str {
        "Convention/NoIsnullFunction"
    }
...
fn find_violations(source: &str, rule_name: &'static str) -> Vec<Diagnostic> {
    ...
    // "ISNULL" is 6 characters
    let keyword = b"ISNULL";
    ...
    while i + kw_len <= len {
        ...
        // Case-insensitive match of "ISNULL"
