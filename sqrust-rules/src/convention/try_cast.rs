...
use sqlparser::ast::{
    CastKind, Expr, FunctionArg, FunctionArgExpr, FunctionArguments, Query, Select, SelectItem,
...
fn walk_expr(
    ...
    match expr {
        Expr::Cast {
            ...
            let occ = counters[0];
            counters[0] += 1;
            let offset = find_occurrence(source, "TRY_CAST", occ);
            let (line, col) = line_col(source, offset);
            diags.push(Diagnostic {
                rule,
                message: "TRY_CAST() is SQL Server/Azure-specific — use CASE WHEN with validation or a dialect macro for portable error-tolerant casting".to_string(),
                line,
                col,
            });
        ...
        Expr::Cast {
            ...
            let occ = counters[1];
            counters[1] += 1;
            let offset = find_occurrence(source, "SAFE_CAST", occ);
            let (line, col) = line_col(source, offset);
            diags.push(Diagnostic {
                rule,
                message: "SAFE_CAST() is BigQuery-specific — use CASE WHEN with validation or a dialect macro for portable error-tolerant casting".to_string(),
                line,
                col,
            });
            walk_expr(inner, source, counters, rule, diags);
...
impl Rule for TryCast {
    ...
    fn check(&self, ctx: &FileContext) -> Vec<Diagnostic> {
        ...
        // counters[0] = try_cast occurrences, counters[1] = safe_cast occurrences
