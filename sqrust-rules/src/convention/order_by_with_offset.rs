...
pub struct OrderByWithOffset;

impl Rule for OrderByWithOffset {
    fn name(&self) -> &'static str {
        "Convention/OrderByWithOffset"
    }
...
fn collect_from_query(query: &Query, ctx: &FileContext, diags: &mut Vec<Diagnostic>) {
    ...
    // Check this query's own OFFSET / ORDER BY.
    ...
    if has_offset && !has_order_by {
        let (line, col) = find_keyword_pos(&ctx.source, "OFFSET");
        diags.push(Diagnostic {
            rule: "Convention/OrderByWithOffset",
            message: "OFFSET without ORDER BY produces non-deterministic results".to_string(),
            line,
            col,
        });
    }
...
fn collect_from_select(select: &Select, ctx: &FileContext, diags: &mut Vec<Diagnostic>) {
    ...
    // WHERE clause — subquery expressions.
    if let Some(selection) = &select.selection {
        collect_from_expr(selection, ctx, diags);
    }

    // SELECT projection — scalar subqueries.
