...
/// Walk an expression, pushing diagnostics for any LEN() function call.
fn walk_expr(
    expr: &Expr,
    source: &str,
        ...
        Expr::Function(func) => {
            ...
            if lower == "len" {
                ...
                *occurrence_counter += 1;
                ...
                let offset = find_occurrence(source, "LEN", occ);
                let (line, col) = line_col(source, offset);
                diags.push(Diagnostic {
                    rule,
                    message: "LEN() is SQL Server-specific — use LENGTH() for portable string length".to_string(),
                    line,
                    col,
                });
            }
