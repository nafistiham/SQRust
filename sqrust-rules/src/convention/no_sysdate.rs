...
pub struct NoSysdate;

const MESSAGE: &str =
    "SYSDATE is Oracle-specific; use CURRENT_DATE or CURRENT_TIMESTAMP instead for portable SQL";

impl Rule for NoSysdate {
    fn name(&self) -> &'static str {
        "Convention/NoSysdate"
    }
...
fn find_violations(source: &str, rule_name: &'static str) -> Vec<Diagnostic> {
    ...
    // "SYSDATE" is 7 characters
    let keyword = b"SYSDATE";
    ...
    while i + kw_len <= len {
        ...
        // Case-insensitive match of "SYSDATE"
        ...
        if !(0..kw_len).all(|k| !skip.contains(&(i + k))) {
            i += 1;
            continue;
        }
