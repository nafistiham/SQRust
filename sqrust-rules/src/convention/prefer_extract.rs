...
pub struct PreferExtract;

/// Dialect-specific date-part functions that should be replaced with EXTRACT().
const FLAGGED_FUNCS: &[&str] = &["year", "month", "day", "hour", "minute", "second"];

const MESSAGE: &str =
    "YEAR()/MONTH()/DAY() are dialect-specific \
     — use EXTRACT(YEAR FROM x) for portable date part extraction";
...
fn walk_expr(
    ...
    match expr {
        Expr::Function(func) => {
            ...
            if FLAGGED_FUNCS.contains(&lower.as_str()) {
                let count = occurrence_counters.entry(lower.clone()).or_insert(0);
                let occ = *count;
                *count += 1;

                let offset = find_occurrence(source, &lower.to_uppercase(), occ);
...
impl Rule for PreferExtract {
    fn name(&self) -> &'static str {
        "Convention/PreferExtract"
    }
