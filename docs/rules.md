...
| `Ambiguous/BetweenReversedBounds` | `BETWEEN` with lower bound greater than upper bound never matches |
| `Ambiguous/CaseNullCheck` | `CASE` branches checking NULL without `IS NULL` |
| `Ambiguous/CaseWhenSameResult` | All `CASE` branches return the same value — the `CASE` is redundant |
...
| `Ambiguous/LikeEscapeChar` | `LIKE` pattern with backslash escape needs an `ESCAPE` clause in standard SQL |
...
| `Ambiguous/OverlappingCaseWhen` | A `WHEN` condition is always TRUE, making later branches unreachable |
...
| `Ambiguous/StringLiteralNewline` | String literal spans lines via an embedded newline; use concatenation instead |
...
| `Ambiguous/UndelimitedDateString` | Date string without delimiters (e.g. `20260101`) is ambiguous |
...
| `Convention/NoIlike` | `ILIKE` is PostgreSQL-specific; use `LOWER(col) LIKE` for portability |
...
| `Convention/NoMoneyType` | `MONEY`/`SMALLMONEY` types are SQL Server–specific; use `DECIMAL` |
...
| `Convention/NoSimilarTo` | `SIMILAR TO` is not universally supported; use `LIKE` or `REGEXP` |
| `Convention/NoSysdate` | `SYSDATE` is Oracle-specific; use `CURRENT_TIMESTAMP` |
| `Convention/NoTableHint` | Table hints (`WITH (NOLOCK)` etc.) are SQL Server–specific |
...
| `Convention/PreferAnsiTrim` | `TRIM`/`LTRIM`/`RTRIM` variants differ across dialects; use the ANSI form |
| `Convention/PreferCoalesceOverNullCase` | `CASE WHEN x IS NULL THEN y ELSE x END` should be `COALESCE(x, y)` |
...
| `Convention/UnnecessaryElseNull` | `ELSE NULL` is redundant in a `CASE`; omit it for the same result |
...
| `Layout/CaseEndNewLine` | `END` of a `CASE` expression should start on its own line |
...
| `Layout/GroupByColumnPerLine` | In multi-line `GROUP BY`, each column should be on its own line |
...
| `Layout/OrderByColumnPerLine` | In multi-line `ORDER BY`, each column should be on its own line |
...
| `Layout/SpaceAroundBitwiseOperator` | Spaces required around bitwise operators (`&`, `|`, `^`) |
...
| `Layout/SpaceAroundModulo` | Spaces required around the modulo operator `%` |
...
| `Layout/WhenOnNewLine` | `WHEN` clause should start on a new line for readability |
...
| `Lint/AlterViewStatement` | `ALTER VIEW` statements should not appear in SQL files |
| `Lint/CallStatement` | `CALL` invokes a stored procedure; consider a `SELECT` or function instead |
...
| `Lint/CreateTriggerStatement` | `CREATE TRIGGER` statements should not appear in SQL files |
...
| `Lint/ExplainStatement` | `EXPLAIN` should not appear in production SQL files |
...
| `Lint/LockTableStatement` | `LOCK TABLE` blocks concurrent access; avoid in analytics SQL |
...
| `Lint/SelfAlias` | `SELECT x AS x` — alias identical to the column it renames |
...
| `Structure/DeeplyNestedCase` | `CASE` expressions nested beyond the configured depth |
| `Structure/DistinctGroupBy` | `DISTINCT` combined with `GROUP BY` |
| `Structure/DuplicateGroupByColumn` | Same column listed more than once in `GROUP BY` |
| `Structure/DuplicateOrderByColumn` | Same column listed more than once in `ORDER BY` |
...
| `Structure/LargeInList` | `IN (...)` list exceeds the configured length; use a temp table or subquery |
...
| `Structure/MultipleStatementsInFile` | File contains multiple statements; consider one statement per file |
...
| `Structure/OrderByInCte` | `ORDER BY` inside a CTE has no effect unless paired with `LIMIT` |
...
| `Structure/UnionColumnAlias` | Column alias in a non-first UNION branch is ignored |
